...
def horas_picos_vs_tranquilas_en_semana(resultado_final):
    ...
    for avenida in resultado_final.items():
        ...
        resultado[nombre_avenida] = {"Horas pico 7-9,12-14,16-19": [0,0], "Horas tranquilas": [0,0]}
        for franja_h in resultado_final[nombre_avenida]['Semana'].items():
            ...
            for parada in resultado_final[nombre_avenida]['Semana'][franja].items():
                cod_parada = parada[0]
                for linea in resultado_final[nombre_avenida]['Semana'][franja][cod_parada].items():
                    cod_linea = linea[0]
                    ...
                    resultado[nombre_avenida][franja][0] = (actual_desviacion * actual_viajes + nueva_desviacion * nuevos_viajes) / (actual_viajes + nuevos_viajes)
        ...
        for dia in resultado_final[nombre_avenida].items():
            ...
            for franja_h in resultado_final[nombre_avenida][tipo_dia].items():
                ...
                for parada in resultado_final[nombre_avenida][tipo_dia][franja].items():
                    cod_parada = parada[0]
                    for linea in resultado_final[nombre_avenida][tipo_dia][franja][cod_parada].items():
                        cod_linea = linea[0]
                        actual_desviacion = resultado[nombre_avenida][tipo_dia][0]
                        actual_viajes = resultado[nombre_avenida][tipo_dia][1]
                        nueva_desviacion = linea[1][0]
                        nuevos_viajes = linea[1][1]
                        resultado[nombre_avenida][tipo_dia][0] = (actual_desviacion * actual_viajes + nueva_desviacion * nuevos_viajes) / (actual_viajes + nuevos_viajes)
                ...
                for parada in resultado_final[nombre_avenida][tipo_dia][franja].items():
                    ...
                    for linea in resultado_final[nombre_avenida][tipo_dia][franja][cod_parada].items():
                        ...
                        if cod_linea == linea_objetivo:
                            ...
                            nueva_desviacion = linea[1][0]
                            nuevos_viajes = linea[1][1]
                            resultado[nombre_avenida][0] = (actual_desviacion * actual_viajes + nueva_desviacion * nuevos_viajes) / (actual_viajes + nuevos_viajes)
            ...
            for franja_h in resultado_final[nombre_avenida][tipo_dia].items():
                ...
                for parada in resultado_final[nombre_avenida][tipo_dia][franja].items():
                    ...
                    for linea in resultado_final[nombre_avenida][tipo_dia][franja][cod_parada].items():
                        nueva_desviacion = linea[1][0]
                        nuevos_viajes = linea[1][1]
                        if nueva_desviacion > DESVIACION_POS_SIGNIFICATIVA_SEGUNDOS or nueva_desviacion < DESVIACION_NEG_SIGNIFICATIVA_SEGUNDOS:
